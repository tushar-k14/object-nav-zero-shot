...
INDOOR_CLASSES = {
    "bed", "chair", "couch", "sofa", "bench",
    "dining table", "table", "desk",
    "toilet", "sink", "refrigerator", "oven", "microwave",
    "tv", "laptop", "fan", "lamp", 
    "stairs", "door", "window", "curtain",
    "painting", "picture",
    # ObjectNav target categories — must not be filtered
    "potted plant", "book", "clock", "cup", "bottle",
    "backpack", "vase",
}
...
class TrackedObject:
    ...
    def decay(self, rate):
        self.confidences = [c * rate for c in self.confidences]
...
class EnhancedSemanticMapperV2:
    ...
    def _match_detection(self, det):
        ...
        for oid, obj in self.tracked_objects.items():
            if obj.class_name.lower() != det.class_name.lower():
                continue
    ...
    def _update_semantic_map(self):
        ...
        for obj in self.get_confirmed_objects().values():
            ...
            footprint *= obs_dilated
            
            semantic_map[y1:y2, x1:x2, cat_idx] = np.maximum(
                semantic_map[y1:y2, x1:x2, cat_idx], footprint
            )
    ...
    def _remove_duplicates(self):
        ...
        for i, (id1, o1) in enumerate(objs):
            ...
            for id2, o2 in objs[i+1:]:
                if id2 in to_remove:
                    continue
                if o1.class_name != o2.class_name or o1.floor_level != o2.floor_level:
                    continue
    ...
    def _frame_nms(self, detections, dist_thresh=0.9):
        kept = []
        ...
        for d in detections:
            ...
            for k in kept:
                if d.class_name != k.class_name:
                    continue
                if d.position_3d is None or k.position_3d is None:
                    continue
                if np.linalg.norm(d.position_3d - k.position_3d) < dist_thresh:
                    keep = False
                    break
            if keep:
                kept.append(d)
    ...
    def update(self, rgb, depth, agent_position, agent_rotation):
        ...
        # --- Decay stale objects ONCE per frame (not per detection) ---
        to_delete = []
        for oid, obj in self.tracked_objects.items():
            frames_since = self.frame_count - obj.last_seen_frame
            if frames_since > 5:
                obj.decay(rate=0.97 ** frames_since)
            if obj.mean_confidence < 0.25:
        ...
        if self.model is not None:
            ...
            if len(results) > 0 and results[0].boxes is not None:
                ...
                for i in range(len(result.boxes)):
                    ...
                    if cls_name not in INDOOR_CLASSES:
                        continue
                ...
                # --- Per-frame NMS to suppress duplicates ---
                self.last_detections = self._frame_nms(self.last_detections)
                    ...
                    if det.position_3d is not None:
                        ...
                        if matched >= 0:
                            self.tracked_objects[matched].add_observation(
                                det.position_3d, det.confidence,
                                det.bbox, self.frame_count
                            )
                        else:
                            new_id = self.next_object_id
                            self.next_object_id += 1
                            obj = TrackedObject(new_id, det.class_name,
                                               floor_level=self.current_floor,
                                               creation_frame=self.frame_count)
                            obj.add_observation(det.position_3d, det.confidence,
                                               det.bbox, self.frame_count)
                            self.tracked_objects[new_id] = obj
                        
                        if det.class_name == 'stairs':
                            self._record_stairs(det.position_3d)
    ...
    def _record_stairs(self, pos):
        for s in self.stairs_detected:
            if np.linalg.norm(np.array(s['position']) - pos) < 2.0:
                return
        # Determine destination floor based on vertical direction
        floor_to = self.current_floor - 1  # default: stairs go down
        if self.last_agent_height is not None and self.floor_base_heights.get(self.current_floor) is not None:
            if self.last_agent_height > self.floor_base_heights[self.current_floor] + 0.5:
                floor_to = self.current_floor + 1  # going up
        self.stairs_detected.append({
            'position': pos.tolist(),
            'map_pos': list(self.occupancy_map._world_to_map(pos[0], pos[2])),
            'radius': 18,
